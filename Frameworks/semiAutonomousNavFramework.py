...
class ANDRRFramework:
    ...
    def __init__(self):
        ...
        self.serOut='/dev/ttyACM0' #Serial port for outputting processed data
        ...
        try:
            self.serOut=serial.Serial(self.serPort, 115200, timeout=10) #Connects to the Arduino over serial
            time.sleep(2)
        ...
        try:
            ...
            self.serIn.mav.request_data_stream_send(master.target_system, master.target_component, mavutil.mavlink.MAV_DATA_STREAM_ALL, 5, 1) # Request all data streams at 5Hz
        except:
            self.serIn=None
...
#Create queues for passing data between parallel processes
dataOutQueue=Queue()
dataInQueue=Queue()
imageOutQueue=Queue()
