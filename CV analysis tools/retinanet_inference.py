...
# Step 3: Load the RetinaNet model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Using TorchVision model hub to load RetinaNet
...
for filepath, sample in zip(filepaths, subset_view):
    ...
    person_detected = any((labels == 1) & (scores > 0.5))  # COCO "person" class is 1 and confidence > 0.5
...
# Print the total number of parameters in the RetinaNet model
